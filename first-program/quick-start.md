...
The environmental variable fluxtion.cacheDIrectory sets the [cache directory](https://fluxtion.gitbook.io/docs/first-program/quick-start#cached-compilation) for outputs of the fluxtion compiler.
...
Executing the jar a second time sees a significant reduction in execution time as the application uses the compiled processor from the first run. The cached classes are loaded internally by the reuseOrBuild method. Using the cached compiled classes gives an almost instant response to the event stream in the application.
